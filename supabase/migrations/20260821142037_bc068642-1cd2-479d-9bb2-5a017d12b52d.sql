...
DECLARE
  ...
  v_tipo_pessoa TEXT;
BEGIN
  SELECT COALESCE(s.tipo_pessoa, 'PF') INTO v_tipo_pessoa
  FROM public.socios s WHERE s.id = NEW.socio_id;

  -- Sócio PJ não sofre tributação (PJ para PJ): não gera alerta
  IF v_tipo_pessoa = 'PJ' THEN
    DELETE FROM public.alertas
    WHERE socio_id = NEW.socio_id AND tipo = 'ALERTA_50K' AND resolvido = false;
    RETURN NEW;
  END IF;
